...
#include "AgentZetSettings.generated.h"

/**
 * AgentZet plugin settings.
 *
 ...
 *
 * Access at runtime via: GetDefault<UAgentZetDeveloperSettings>()
 */
DECLARE_MULTICAST_DELEGATE_OneParam(FAgentZetSettingsChangedDelegate, FName /*PropertyName*/);

UCLASS(config = EditorPerProjectUserSettings, meta = (DisplayName = "AgentZet"))
class AGENTZETCORE_API UAgentZetDeveloperSettings : public UDeveloperSettings
{
	GENERATED_BODY()

public:
	UAgentZetDeveloperSettings();

	static FAgentZetSettingsChangedDelegate OnSettingsChanged;
	 ...
	 *  AgentZet supports Anthropic (Claude), OpenAI (GPT/o-series), Google (Gemini),
	...
	EAgentZetProvider ActiveProvider;
	...
	EAgentZetContextWindow ContextWindow;
	...
	EAgentZetReasoningEffort OpenAiReasoningEffort;
	...
	/** Security mode controlling what capabilities AgentZet is allowed to use.
	...
	EAgentZetSecurityMode SecurityMode;
	 ...
	 *  Config/DefaultEditor.ini, Config/DefaultGame.ini, .gitignore, .AgentZetignore
	 *
	 *  Write attempts to protected paths always return EAgentZetRiskLevel::Critical. */
	...
	static const UAgentZetDeveloperSettings* Get();
	...
	static FString ModelEnumToApiString(EAgentZetClaudeModel Model);
	bool IsToolCategoryAllowed(EAgentZetActionCategory Category) const;
