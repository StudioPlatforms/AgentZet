...
FAgentZetBlueprintActions::FAgentZetBlueprintActions() {}
FAgentZetBlueprintActions::~FAgentZetBlueprintActions() {}

FName FAgentZetBlueprintActions::GetActionName() const { return FName(TEXT("Blueprint")); }
FText FAgentZetBlueprintActions::GetDisplayName() const { return FText::FromString(TEXT("Blueprint Actions")); }
EAgentZetActionCategory FAgentZetBlueprintActions::GetCategory() const { return EAgentZetActionCategory::Blueprint; }
EAgentZetRiskLevel FAgentZetBlueprintActions::GetDefaultRiskLevel() const { return EAgentZetRiskLevel::Medium; }
bool FAgentZetBlueprintActions::CanUndo() const { return true; }
bool FAgentZetBlueprintActions::UndoAction() { return false; }

TArray<FString> FAgentZetBlueprintActions::GetSupportedToolNames() const
{
	return {
...
bool FAgentZetBlueprintActions::ValidateParams(const TSharedRef<FJsonObject>& Params, TArray<FString>& OutErrors) const
{
...
FAgentZetActionPlan FAgentZetBlueprintActions::PreviewAction(const TSharedRef<FJsonObject>& Params)
{
...
FAgentZetActionResult FAgentZetBlueprintActions::ExecuteAction(const TSharedRef<FJsonObject>& Params)
{
	FScopedTransaction Transaction(FText::FromString(TEXT("AgentZet Blueprint Action")));
...
bool FAgentZetBlueprintActions::CompileAndReport(UBlueprint* Blueprint, FAgentZetActionResult& Result, bool bSkipGC)
{
	check(Blueprint);
...
UEdGraph* FAgentZetBlueprintActions::FindOrCreateEventGraph(UBlueprint* Blueprint, const FString& GraphName)
{
	check(Blueprint);
...
USCS_Node* FAgentZetBlueprintActions::FindSCSNodeByName(UBlueprint* Blueprint, const FString& NodeName)
{
...
FString FAgentZetBlueprintActions::ResolveT3DPlaceholders(const FString& T3DText)
{
...
bool FAgentZetBlueprintActions::DetectInfiniteLoopRisk(UBlueprint* Blueprint, TArray<FString>& OutWarnings) const
{
...
FString FAgentZetBlueprintActions::BuildBlueprintInfoJson(UBlueprint* Blueprint)
{
...
bool FAgentZetBlueprintActions::ResolveEventMapping(const FString& EventName, FName& OutFunctionName, UClass*& OutOwnerClass)
{
	// Actor events
...
FAgentZetActionResult FAgentZetBlueprintActions::ExecuteCreateBlueprint(const TSharedRef<FJsonObject>& Params, FAgentZetActionResult& Result)
{
	FString AssetPath      = Params->GetStringField(TEXT("asset_path"));
	...
	UE_LOG(LogAgentZet, Log, TEXT("BlueprintActions: Created Blueprint '%s' with parent '%s'"), *AssetPath, *ParentClassName);
		...
		for (const TSharedPtr<FJsonValue>& CompValue : *ComponentsArray)
		{
			...
			if (CompClass && NewBlueprint->SimpleConstructionScript)
			{
				...
				if (NewNode)
				{
					...
					UE_LOG(LogAgentZet, Log, TEXT("BlueprintActions: Added inline component '%s' (%s)"), *CompName, *CompClassName);
				}
			}
			else
			{
...
FAgentZetActionResult FAgentZetBlueprintActions::ExecuteAddComponent(const TSharedRef<FJsonObject>& Params, FAgentZetActionResult& Result)
{
	FString AssetPath    = Params->GetStringField(TEXT("asset_path"));
...
FAgentZetActionResult FAgentZetBlueprintActions::ExecuteAddVariable(const TSharedRef<FJsonObject>& Params, FAgentZetActionResult& Result)
{
	FString AssetPath = Params->GetStringField(TEXT("asset_path"));
...
FAgentZetActionResult FAgentZetBlueprintActions::ExecuteAddFunction(const TSharedRef<FJsonObject>& Params, FAgentZetActionResult& Result)
{
	FString AssetPath    = Params->GetStringField(TEXT("asset_path"));
...
FAgentZetActionResult FAgentZetBlueprintActions::ExecuteAddEventHandler(const TSharedRef<FJsonObject>& Params, FAgentZetActionResult& Result)
{
	FString AssetPath  = Params->GetStringField(TEXT("asset_path"));
...
FAgentZetActionResult FAgentZetBlueprintActions::ExecuteCompileBlueprint(const TSharedRef<FJsonObject>& Params, FAgentZetActionResult& Result)
{
	FString AssetPath = Params->GetStringField(TEXT("asset_path"));
...
FAgentZetActionResult FAgentZetBlueprintActions::ExecuteSetDefaults(const TSharedRef<FJsonObject>& Params, FAgentZetActionResult& Result)
{
	FString AssetPath = Params->GetStringField(TEXT("asset_path"));
	...
	for (const auto& Pair : (*DefaultsObj)->Values)
	{
		...
		if (Pair.Value->TryGetString(ValueStr))
		{
			...
			UE_LOG(LogAgentZet, Log, TEXT("BlueprintActions: Set default '%s' = '%s' on '%s'"), *FullKey, *ValueStr, *TargetObject->GetClass()->GetName());
		}
		else
		{
...
FAgentZetActionResult FAgentZetBlueprintActions::ExecuteSetComponentProperties(const TSharedRef<FJsonObject>& Params, FAgentZetActionResult& Result)
{
	FString AssetPath     = Params->GetStringField(TEXT("asset_path"));
	...
	if (Params->TryGetStringField(TEXT("static_mesh"), StaticMeshPath) && !StaticMeshPath.IsEmpty())
	{
		...
		else
		{
			...
			if (Mesh)
			{
				...
				UE_LOG(LogAgentZet, Log, TEXT("BlueprintActions: Set StaticMesh '%s' on component '%s'"), *StaticMeshPath, *ComponentName);
			}
			else
			{
	...
	if (Params->TryGetStringField(TEXT("skeletal_mesh"), SkeletalMeshPath) && !SkeletalMeshPath.IsEmpty())
	{
		...
		else
		{
			...
			if (Mesh)
			{
				...
				UE_LOG(LogAgentZet, Log, TEXT("BlueprintActions: Set SkeletalMesh '%s' on component '%s'"), *SkeletalMeshPath, *ComponentName);
			}
			else
			{
...
FAgentZetActionResult FAgentZetBlueprintActions::ExecuteInjectNodesT3D(const TSharedRef<FJsonObject>& Params, FAgentZetActionResult& Result)
{
	FString AssetPath = Params->GetStringField(TEXT("asset_path"));
	FString T3DText;
	...
	UE_LOG(LogAgentZet, Log, TEXT("BlueprintActions: Injecting T3D nodes into graph '%s' of '%s'"), *GraphName, *AssetPath);
...
FAgentZetActionResult FAgentZetBlueprintActions::ExecuteGetBlueprintInfo(const TSharedRef<FJsonObject>& Params, FAgentZetActionResult& Result)
{
	FString AssetPath = Params->GetStringField(TEXT("asset_path"));
...
FAgentZetActionResult FAgentZetBlueprintActions::ExecuteConnectPins(const TSharedRef<FJsonObject>& Params, FAgentZetActionResult& Result)
{
	FString AssetPath;
	FString GraphName   = TEXT("EventGraph");
	FString SourceNode;
...
FString FAgentZetBlueprintActions::BuildPinAuditReport(const TSet<UEdGraphNode*>& Nodes)
{
...
FString FAgentZetBlueprintActions::BuildCompactConnectionReport(const TSet<UEdGraphNode*>& Nodes, const FString& GraphName)
{
...
void FAgentZetBlueprintActions::ResolvePinType(const FString& TypeName, FEdGraphPinType& OutPinType)
{
	...
	else
	{
		...
		else
		{
			...
			else
			{
				...
				UE_LOG(LogAgentZet, Warning, TEXT("BlueprintActions: Unresolved pin type '%s' — using wildcard. Check the type name spelling."), *TypeName);
			}
		}
		}
	}
	...
	FAgentZetActionResult FAgentZetBlueprintActions::ExecuteAddEnhancedInputNode(
		const TSharedRef<FJsonObject>& Params, FAgentZetActionResult& Result)
	{
		...
		UE_LOG(LogAgentZet, Log, TEXT("BlueprintActions: Added Enhanced Input node '%s' for IA '%s' in '%s'"),
	...
	FAgentZetActionResult FAgentZetBlueprintActions::ExecuteVerifyConnections(const TSharedRef<FJsonObject>& Params, FAgentZetActionResult& Result)
	{
		FString AssetPath;
		...
		UE_LOG(LogAgentZet, Log, TEXT("BlueprintActions: VerifyConnections completed for '%s' — %d issues, %d repairs, %d unresolved"),
...
void FAgentZetBlueprintActions::AutoLayoutNodes(
	...
	int32 LayerSpacingX, int32 NodeSpacingY)
{
	...
	UE_LOG(LogAgentZet, Log, TEXT("BlueprintActions: Auto-layout applied to %d nodes across %d layers"),
...
bool FAgentZetBlueprintActions::PreFlightValidateT3D(
...
FAgentZetActionResult FAgentZetBlueprintActions::ExecuteSetNodePinDefault(const TSharedRef<FJsonObject>& Params, FAgentZetActionResult& Result)
{
	FString AssetPath = Params->GetStringField(TEXT("asset_path"));
	FString NodeName;
...
FAgentZetActionResult FAgentZetBlueprintActions::ExecuteDeleteNodes(const TSharedRef<FJsonObject>& Params, FAgentZetActionResult& Result)
{
	FString AssetPath = Params->GetStringField(TEXT("asset_path"));
	FString GraphName = TEXT("EventGraph");
	Params->TryGetStringField(TEXT("graph_name"), GraphName);
	...
	for (UEdGraphNode* Node : NodesToRemove)
	{
		...
		UE_LOG(LogAgentZet, Log, TEXT("BlueprintActions: Deleted node '%s' (%s) from graph '%s' of '%s'"),
