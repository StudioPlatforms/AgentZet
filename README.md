<p align="center">
  <h1 align="center"> AgentZet — AI Developer for Unreal Engine</h1>
  <p align="center">
  ...
  <p align="center">
    A <a href="https://studioplatforms.eu"><strong>Studio Platforms</strong></a> project
  </p>
  <p align="center">
...
- **Conversation Persistence** — Every task auto-saves to `Saved/AgentZet/Tasks/<uuid>/` with dual files: `api_history.json` (full message history for API replay) and `ui_messages.json` (display-optimized messages for instant UI restore). A `task_index.json` metadata index tracks all tasks with titles, timestamps, token counts, and cost.
...
- **Multimodal Viewport Vision** — AgentZet isn't blind. Using Vision-Language Models (VLMs), the AI can trigger the `capture_viewport` tool to visually analyze the Editor. It can build a UI, "look" at the viewport, and autonomously correct misaligned anchor points or bad lighting setups.
...
- **Multi-Tab Conversations** — Work on multiple tasks simultaneously with full state persistence across editor restarts. Each task saved to `Saved/AgentZet/Tasks/<uuid>/` with dual-file model (API history + UI messages)
...
- **`.AgentZetignore`** — Gitignore-style pattern file to hide files/directories from the AI. Auto-created with sensible defaults on first run. Live-reloads on changes.
...
- **Execution Journal** — Append-only audit trail of every tool action with SHA-1 pre/post file hashes for deterministic state verification. Saved to `Saved/AgentZet/ExecutionLog_YYYYMMDD.json`.
...
Regex-powered file content search with 2-line context before/after each match. Formats output in ripgrep style. Skips binary files, respects `.AgentZetignore` patterns.
...
###  Gameplay Ability System (GAS) Tools (5 tools)
...
- **Unreal Engine 5.7.4+ users:** **Visual Studio 2026** must be installed for the plugin to build and work correctly
...
1. **Clone the repository** into your project's `Plugins` folder:
   ...
   git clone https://github.com/PRQELT/AgentZet.git
   ```
...
4. **Enable the plugin** — It should be enabled by default. If not: Edit → Plugins → search "AgentZet" → Enable → Restart

5. **Configure your provider** — Edit → Project Settings → Plugins → AgentZet:
...
4. In AgentZet settings: set Provider to **Ollama (Local)**, Base URL to `http://localhost:11434`, Model ID to your pulled model name

**LM Studio**:
1. Download from [lmstudio.ai](https://lmstudio.ai)
2. Load a model in the LM Studio UI
3. Enable **Local Server** (starts on port 1234 by default)
4. In AgentZet settings: set Provider to **LM Studio (Local)**, Base URL to `http://localhost:1234`, Model ID as shown in LM Studio
...
4. In AgentZet settings (Edit → Project Settings → Plugins → AgentZet → API | Azure OpenAI):
...
2. Extract the contents into `YourProject/Plugins/AgentZet/`
...
1. **Open the AgentZet panel**: Window → AgentZet (or use the toolbar button)
...
AgentZet is organized into 5 modules with clear dependency boundaries:

```
AgentZet/
├── Source/
│   ├── AgentZetCore/        # Types, settings, project context, interfaces
│   │
│   ├── AgentZetLLM/         # LLM integration layer
...
│   │
│   ├── AgentZetEngine/      # Orchestration and infrastructure
...
│   │   ├── IgnoreController   — .AgentZetignore pattern matching
...
│   │
│   ├── AgentZetActions/     # Tool executors (one per domain)
...
│   │
│   └── AgentZetUI/          # Slate widget layer
│       ├── SAgentZetMainPanel      — Central orchestrator (tabs, agentic loop, tool dispatch)
│       ├── SAgentZetChatView       — Message list with streaming support
│       ├── SAgentZetMessage        — Individual message rendering (markdown, thinking blocks)
│       ├── SAgentZetCodeBlock      — Syntax-highlighted code display with copy button
│       ├── SAgentZetDiffViewer     — Unified diff visualization
│       ├── SAgentZetInputArea      — Multi-line input with @ and / autocomplete + Send/Stop toggle
│       ├── SAgentZetContextBar     — Progress bar + token/cost display
│       ├── SAgentZetCheckpointPanel— Checkpoint list, restore, diff buttons
│       ├── SAgentZetHistoryPanel   — Past conversation browser
│       ├── SAgentZetTodoList       — Live task checklist
│       ├── SAgentZetPlanPreview    — AI plan display
│       ├── SAgentZetFollowUpBar    — Suggested follow-up actions
│       ├── SAgentZetFileChangesPanel — Modified files tracker
│       ├── SAgentZetAssetPreview   — Inline asset thumbnail display
│       └── SAgentZetProgress       — Operation progress overlay
│
├── Resources/
│   ├── SystemPrompt/         # AI system prompt with rules and workflow
│   └── ToolSchemas/          # 24 JSON tool definition files
│
└── AgentZet.uplugin        # Plugin descriptor (5 modules)
```
...
- **Shadow Git Checkpoints** — A separate git repository in `Saved/AgentZet/Checkpoints/[SessionId]/` mirrors tracked project files. Each tool execution batch creates a commit. Restore to any checkpoint without affecting the project's own git history.
...
All settings are in **Edit → Project Settings → Plugins → AgentZet**:
...
AgentZet uses a multi-step verification process to ensure AI-generated changes are correct. The agentic loop doesn't stop at "it compiles" — it progressively validates work through increasingly rigorous checks:
...
- **`.AgentZetignore`** — Gitignore-style file in project root controls AI file access. Auto-created with sensible defaults.
...
Every AgentZet tool follows the same pattern. To add a new tool:

1. **Header** — Create `Source/AgentZetActions/Public/YourDomain/AgentZetYourActions.h`
   - Inherit from `IAgentZetActionExecutor` (defined in `AgentZetInterfaces.h`)
   - Implement all virtual methods: `GetActionName`, `GetSupportedToolNames`, `ExecuteAction`, etc.

2. **Implementation** — Create `Source/AgentZetActions/Private/YourDomain/AgentZetYourActions.cpp`
   - Route tool names via `tool_name` field in `ExecuteAction`
   - Return results via `FAgentZetActionResult` with `bSuccess`, `ResultMessage`, `Errors`, `ModifiedAssets`
...
4. **Settings Toggle** — Add a `bool bEnableYourTools` UPROPERTY to `UAgentZetDeveloperSettings`
...
5. **Register** — In `SAgentZetMainPanel::RegisterExecutors()`:
   - `#include "YourDomain/AgentZetYourActions.h"`
   - `ActionRouter->RegisterExecutor(MakeShared<FAgentZetYourActions>());`
...
- A [**Studio Platforms**](https://studioplatforms.eu) project. For inquiries, contact [ionutvisan@studioplatforms.eu](mailto:ionutvisan@studioplatforms.eu).
